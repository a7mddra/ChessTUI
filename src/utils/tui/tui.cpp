           ...
           const char &clr, bool nline)
{
    ...
    {
        output = std::string(padd(lines[i]), ' ');

        switch (clr)
        {
        case 'p':
            output += color::purple(lines[i]);
            break;
        case 'o':
            output += color::orange(lines[i]);
            break;
        case 'y':
            output += color::yellow(lines[i]);
            break;
        case 'g':
            output += color::green(lines[i]);
            break;
        case 'b':
            output += color::blue(lines[i]);
            break;
        case 'r':
            output += color::red(lines[i]);
            break;
        default:
            output += lines[i];
            break;
